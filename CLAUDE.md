...
- - **Phase status:** Phase 1 Sprint 4 (Weeks 7-8), mid-execution. Phase 0 closed at end of Sprint 3. Sprint 4 cluster status: A1+A2+A3 design memos signed off; B1, C1.1, C1.2, C2.1, C2.2, C3 (scaffolding) committed; C3.h (50-epoch baseline run) pending hand-off; C1.3 not started (C1.3-lite design locked); C4 + Z not started.
...
- Sprint 4 substrate (Phase 1 ML detector + synthetic data):
  ...
  - C3 training pipeline + Tier-1 evaluation:
    - `scripts/build_training_dataset.py` produces bulk synthetic datasets; v1 at `/Users/keith/Documents/data/training_dataset_v1/` (500 clips, full A1 priors + biologicals → ~7k usable patches, ~39% positive).
  ...
  - **C3.h carry-over (50-epoch baseline run, hand-off to Keith on MPS or cloud GPU)**: command in the C3 commit message. Results commit lands under `artifacts/sprint4_baseline/` with final P/R/F1 tables.
