...
**Status:** Draft pending CEO sign-off (Sprint 4 Cluster A2 deliverable).
...
The detection task: persistent narrow tonal lines on LOFAR spectrograms. Lines appear as thin horizontal stripes (constant frequency, persistent over seconds-to-minutes), sometimes with slow drift. Targets in the LOFAR primary view are 3 Hz–1000 Hz; per-frame frequency resolution is ~1.95 Hz at the configured FFT (n_fft=16384 at sr=32 kHz); time resolution ~128 ms.

The literature digest (Han et al. 2020 DeepLofargram; Kim & Yoon 2019/2020; Bergler et al. 2022 ANIMAL-SPOT; Schall et al. 2024 baleen benchmark; Peng et al. 2025; Shit et al. 2021 clDice; Lin et al. 2017 focal loss; Ju et al. 2022 line enhancer) surfaced a strong empirical convergence on convolutional architectures with small-network backbones for sparse-positive spectrogram detection. This memo locks the architecture choice; A1 handles synthetic data; A3 handles evaluation.

## 2. Alternatives considered

**(a) Patch-based binary classification + localization head (Kim & Yoon 2019/2020 lineage).** 256×256 LOFAR-gram patches. Backbone (ResNet-18 / EfficientNet-B0) → binary head ("contains line / does not") + regression head (line center frequency + time extent within patch). Kim & Yoon 2020 reports 96.2% precision / 99.5% recall at 184 ms/inference on LOFARgrams. PCD v3 §7.3 baseline. **Chosen as primary architecture.**
...
### 3.1 Primary architecture — patch-based ResNet-18 binary + localization

Mirrors Kim & Yoon 2020 and ANIMAL-SPOT.
...
- **Heads:**
  ...
  - **Localization head.** Active only on positive patches. Outputs: `(f_center_within_patch, t_start_within_patch, t_end_within_patch)` as normalized coordinates [0, 1]. Loss: smooth-L1 (Huber) on each component. Combined loss `L = L_classification + λ · L_localization` with λ=1 default; ablation may tune.
...
Full evaluation methodology lives in `sim_to_real_evaluation.md` (A3 memo). Sprint 4 specifically:

- **Synthetic test set:** held-out 20% of synthetic dataset. Report per-SNR-bucket precision/recall/F1; report line-IoU (custom metric defined in A3 memo: temporal-extent overlap × 1.0 if frequency centers within 2 bins, 0.5 if within 4 bins, 0 otherwise).
...
2. **Patch framing truncates long-persistence lines.** A line lasting 60 seconds spans many patches. Sprint 4 C2 inference stitches patch-level detections into recording-level lines via downstream connected-component analysis. **Mitigation:** 75% overlap stride at inference; downstream stitching tested on synthetic with known long-persistence injections.
3. **Drift handling in patch framing.** Patches are temporally short (~32 seconds at 50% overlap); slow drift over many seconds appears as a drifting line WITHIN one patch. **Mitigation:** localization-head training on synthetic data with controlled drift rates teaches the model to allow within-patch drift. Cross-patch drift handled by downstream stitching.
...
6. **PyTorch ImageNet-pretrained ResNet-18 weights are color-image-trained.** Adapting to single-channel spectrogram input via input-layer surgery may waste pretrained signal. **Mitigation:** option A — replicate the spectrogram across 3 input channels (free; preserves pretrained weights). Option B — pretrain ResNet-18 from scratch on synthetic spectrograms first. Default option A; ablate option B in Sprint 5 if Sprint 4 numbers underwhelm.
...
8. **Localization regression on patch coordinates.** Normalizing localization to [0,1] within-patch coordinates loses cross-patch global frequency reference. **Mitigation:** the patch's global frequency offset is metadata at inference; recovered global frequency = patch_offset + within-patch normalized × patch_freq_extent. Handled in inference assembly, not in the model itself.
...
- Nur Korkmaz, B. et al. (2023). "Automated detection of dolphin whistles with convolutional networks and transfer learning." *Frontiers in AI* 6:1099022. — YOLO-style spectrogram detection; not chosen but informed alternative (c) rejection rationale.
...
2. **Primary architecture:** ResNet-18 backbone, 256×256 patches, 50% overlap at training / 75% at inference. Binary classification head (focal loss γ=2) + localization regression head (smooth-L1). Pretrained ImageNet weights, 3-channel input via spectrogram replication (option A; ablate scratch-init in Sprint 5).
...
7. **Inference assembly:** patch-level detections → recording-level lines via connected-component / persistence-aware stitching across patches. Implementation in `ml.py` as `assemble_lines_from_patches()`.
...
Date:
Decisions locked: §8 items 1-10 [accepted | revised: ...]
Comments:
```

After sign-off, this memo becomes the binding spec for Sprint 4 Cluster C2 (ML detection model implementation), C3 (initial training), and C4 (classical-vs-ML parallel smoke test).
