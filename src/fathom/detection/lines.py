...
(detection.peaks), and persistence filtering (detection.persistence) into a
single `detect_lines(gram, config, ...)` call. Populates LineOfInterest
records, publishes Topic.LINE_DETECTED on the event bus, returns the list.

Per Sprint2_Plan §3, the library function's side-effect surface is bounded to
event publishing — `lines.jsonl` writing and audit sidecars live in the
sanity-check script (Cluster 4), not here.
"""
...
class DetectionConfig:
    """Sprint 2 classical line-detection parameters.
...
def detect_lines(
    ...
    *,
    ...
    Steps:
    1. Re-derive a TPSW-normalized power-dB array from `gram.power_db`. The
       gram's `normalized_power_db` is the single-pass form retained for display
       continuity with Sprint 1; detection runs its own TPSW pass.
    2. Run the configured peak detector (`per_bin` or `two_d`).
    3. Aggregate peaks into PersistentLines via the persistence filter.
    4. Build a LineOfInterest for each PersistentLine. Publish each on
       `Topic.LINE_DETECTED` (defaulting to the process-global event bus when
       `bus` is None). Return the list.
    """
    # 1. TPSW two-pass.
    ...
    # 4. LineOfInterest assembly + event publishing.
    ...
    LOG.info("detected %d persistent line(s) on array %s", len(lines), array_id)
    return lines